...
long U_strtol(const char *s, unsigned len, const char **endp, int *err)
{
    ...
    for (; i < len; i++)
    {
        ...
        ch = ch - '0';
        result *= 10;
        e |= (result + ch < result) ? 2 : 0; /* overflow */
...
#if defined(__LP64__) || defined(__lp64__) /* GCC, clang, TCC, IAR, ... */
    if (result > 9223372036854775807ul) /* 64-bit */
    {
        if      (*s != '-')                      e |= 2; /* overflow */
        else if (result > 9223372036854775808ul) e |= 4; /* underflow */
    }
#else /* on MSVC long is 32-bit also on x64 */
    if (result > 2147483647) /* 32-bit */
    {
        if      (*s != '-')           e |= 2; /* overflow */
        else if (result > 2147483648) e |= 4; /* underflow */
    }
#endif
